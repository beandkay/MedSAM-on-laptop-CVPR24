...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x, adapter=True):
        H, W = self.input_resolution
        B, L, C = x.shape
        ...
        if adapter:
            x = self.local_conv(x) + self.Adapter(x)
        else:
            x = self.local_conv(x)
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x, adapter=True):
        # x: (N, C, H, W)
    ...
    def forward(self, x, adapter=True):
        x, interm_embeddings = self.forward_features(x, adapter)
        return x, interm_embeddings